...
<h3 id="diff-focused-analysis">Diff-Focused Analysis</h3>
<p>DryRun Security scans only the files and functions modified in the pull request, along with the surrounding code context needed to evaluate them. This targeted approach keeps scan times fast and findings relevant to the change at hand.</p>

<h3 id="differential-comparison">Differential Comparison</h3>
<p>Both the target branch and the PR branch are analyzed. DryRun Security compares the two result sets and surfaces only <em>new</em> findings introduced by the change, so developers are never distracted by pre-existing issues.</p>

<h3 id="parallel-security-agents">Parallel Security Agents</h3>
<p>DryRun Security's specialized agents - SAST, SCA, Secrets, IaC, and Custom Code Policies - execute simultaneously on the changed code. Parallel execution means comprehensive coverage without sequential bottlenecks.</p>

<h3 id="ai-validation">AI Validation</h3>
<p>Every finding is validated in the context of the actual diff using model-independent verification. Findings are cross-checked across multiple AI models to reduce false positives and ensure each reported issue represents genuine risk.</p>
...
<ul>
  <li><strong>SAST (Code Security Agents)</strong> - Specialized analyzers for SQL injection, XSS, IDOR, SSRF, secrets exposure, and dozens of other vulnerability classes. Each agent traces data flow through the changed code paths.</li>
  <li><strong>SCA (Software Composition Analysis)</strong> - Detects vulnerable dependencies introduced or updated in the PR, cross-referencing known CVEs and advisories.</li>
  <li><strong>Secrets</strong> - Identifies hardcoded credentials, API keys, tokens, and private keys in the diff, with context-aware filtering to distinguish real secrets from test fixtures.</li>
  <li><strong>IaC (Infrastructure as Code)</strong> - Scans Terraform, CloudFormation, Kubernetes manifests, Helm charts, Dockerfiles, and GitHub Actions for misconfigurations and security risks.</li>
</ul>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td><strong>Neutral / Timeout</strong></td>
      <td>The scan completed but could not produce a definitive result, or it exceeded the time limit. Review the summary comment for details.</td>
    </tr>
    <tr>
      <td><strong>Skipped</strong></td>
      <td>The agent was disabled for this repository's configuration, or no relevant files were changed.</td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>PR comments, inline annotations, check statuses</td>
...
<h2 id="adoption-guide">Adoption Guide</h2>

<p>Get started with DryRun Security PR scanning in three steps:</p>

<ol>
  <li><strong>Install DryRun Security</strong> - Connect your GitHub or GitLab organization by following the <a href="./quick-start.html">Quick Start</a> guide. Scanning begins automatically on the next pull request.</li>
  <li><strong>Configure PR blocking</strong> - Optionally enable merge gates so that PRs with findings above your severity threshold cannot be merged. See <a href="./pr-blocking.html">PR Blocking</a> for setup instructions.</li>
  <li><strong>Tune your configuration</strong> - Customize which agents run, attach Custom Code Policies, and adjust blocking thresholds per repository. See <a href="./pr-scanning-configuration.html">PR Scanning Configuration</a> for the full walkthrough.</li>
</ol>
''',
}
